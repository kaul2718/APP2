...
import AgregarTipoEquipoModal from "@/components/modals/AgregarTipoEquipoModal";
import { useTipoEquipo, TipoEquipo } from "@/hooks/useTipoEquipo";
import { apiRequest } from "@/lib/api";

export default function TipoEquipoComponent() {
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [tableRefreshKey, setTableRefreshKey] = useState(0);
    const tipoEquipoHook = useTipoEquipo();
    const { totalItems, fetchTiposEquipo } = tipoEquipoHook;
    const [allTiposForStats, setAllTiposForStats] = useState<TipoEquipo[]>([]);
    ...
    useEffect(() => {
        const loadAllForStats = async () => {
            ...
            try {
                const data = await apiRequest<any>('/tipos-equipo/all?limit=1000&page=1&includeInactive=true', {}, session);
    ...
    return (
        <div className="space-y-6">
            <PageBreadcrumb pageTitle="Gestión de Tipos de Equipo" />

            {/* Quick Stats Section */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <StatCard 
                    icon={<DevicePhoneMobileIcon className="w-6 h-6 text-brand-500" />} 
                    label="Total Tipos" 
                    value={stats.total} 
                    delay={0.1}
                />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white">Catálogo de Tipos de Equipo</h2>
                </div>
            ...
            >
                <div className="p-0">
                    <TipoEquipoTable 
                        key={tableRefreshKey} 
                        tiposEquipoHook={tipoEquipoHook}
                    />
                </div>
            </motion.div>

            <AgregarTipoEquipoModal
                isOpen={isAddModalOpen}
                onClose={() => setIsAddModalOpen(false)}
                onSuccess={() => {
                    setTableRefreshKey((prev) => prev + 1);
                    tipoEquipoHook.fetchTiposEquipo();
                }}
            />
        </div>
    );
