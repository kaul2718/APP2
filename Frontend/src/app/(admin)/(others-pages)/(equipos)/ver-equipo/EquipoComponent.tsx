...
export default function EquipoComponent() {
    ...
    const equipoHook = useEquipos();
    const { totalItems, fetchEquipos } = equipoHook;
    const [allEquiposForStats, setAllEquiposForStats] = useState<Equipo[]>([]);
    ...
    useEffect(() => {
        const loadAllForStats = async () => {
            ...
            try {
                const data = await apiRequest<any>('/equipos/all?limit=1000&page=1&includeInactive=true', {}, session);
                if (data && Array.isArray(data.items)) {
                    setAllEquiposForStats(data.items);
                }
            } catch (error) {
    ...
    // Stats calculations
    const stats = useMemo(() => {
        const total = allEquiposForStats.length;
        const active = allEquiposForStats.filter(e => e.estado).length;
        const inactive = allEquiposForStats.filter(e => !e.estado).length;
        return { total, active, inactive };
    }, [allEquiposForStats]);

    return (
        <div className="space-y-6">
            <PageBreadcrumb pageTitle="Gestión de Equipos" />

            {/* Quick Stats Section */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <StatCard 
                    icon={<ComputerDesktopIcon className="w-6 h-6 text-brand-500" />} 
                    label="Total Equipos" 
                    value={stats.total} 
                    delay={0.1}
                />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white">Inventario de Equipos</h2>
                </div>
