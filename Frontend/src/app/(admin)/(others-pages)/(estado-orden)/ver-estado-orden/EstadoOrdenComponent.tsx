...
import AgregarEstadoOrdenModal from "@/components/modals/AgregarEstadoOrdenModal";
import { useEstadoOrden, EstadoOrden } from "@/hooks/useEstadoOrden";
import { apiRequest } from "@/lib/api";

export default function EstadoOrdenComponent() {
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [tableRefreshKey, setTableRefreshKey] = useState(0);
    const estadoOrdenHook = useEstadoOrden();
    const { totalItems, fetchEstadosOrden } = estadoOrdenHook;
    const [allEstadosForStats, setAllEstadosForStats] = useState<EstadoOrden[]>([]);
    ...
    useEffect(() => {
        const loadAllForStats = async () => {
            ...
            try {
                const data = await apiRequest<any>('/estados-orden/all?limit=1000&page=1&includeInactive=true', {}, session);
    ...
    return (
        <div className="space-y-6">
            <PageBreadcrumb pageTitle="Gestión de Estados de Orden" />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white">Catálogo de Estados</h2>
                </div>
            ...
            >
                <div className="p-0">
                    <EstadoOrdenTable 
                        key={tableRefreshKey} 
                        estadoOrdenHook={estadoOrdenHook}
                    />
                </div>
            </motion.div>

            <AgregarEstadoOrdenModal
                isOpen={isAddModalOpen}
                onClose={() => setIsAddModalOpen(false)}
                onSuccess={() => {
                    setTableRefreshKey((prev) => prev + 1);
                    estadoOrdenHook.fetchEstadosOrden();
                }}
            />
        </div>
    );
