...
import { useModelo, Modelo } from "@/hooks/useModelo";
import { apiRequest } from "@/lib/api";

export default function ModeloComponent() {
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [tableRefreshKey, setTableRefreshKey] = useState(0);
    const modeloHook = useModelo();
    const { totalItems, fetchModelos } = modeloHook;
    const [allModelosForStats, setAllModelosForStats] = useState<Modelo[]>([]);
    ...
    useEffect(() => {
        const loadAllForStats = async () => {
            ...
            try {
                const data = await apiRequest<any>('/modelos/all?limit=1000&page=1&includeInactive=true', {}, session);
                if (data && Array.isArray(data.items)) {
                    setAllModelosForStats(data.items);
                }
            } catch (error) {
    ...
    // Stats calculations
    const stats = useMemo(() => {
        const total = allModelosForStats.length;
        const active = allModelosForStats.filter(m => m.estado).length;
        const inactive = allModelosForStats.filter(m => !m.estado).length;
        return { total, active, inactive };
    }, [allModelosForStats]);

    return (
        <div className="space-y-6">
            <PageBreadcrumb pageTitle="Gestión de Modelos" />

            {/* Quick Stats Section */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <StatCard 
                    icon={<Square3Stack3DIcon className="w-6 h-6 text-brand-500" />} 
                    label="Total Modelos" 
                    value={stats.total} 
                    delay={0.1}
                />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white">Catálogo de Modelos</h2>
                </div>
