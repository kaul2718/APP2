...
import { useTipoActividadTecnica, TipoActividadTecnica } from "@/hooks/useTipoActividadTecnica";
import { apiRequest } from "@/lib/api";

export default function TipoActividadTecnicaComponent() {
    const [isAddModalOpen, setIsAddModalOpen] = useState(false);
    const [tableRefreshKey, setTableRefreshKey] = useState(0);
    const hook = useTipoActividadTecnica();
    const { fetchTipos } = hook;
    const [allTiposForStats, setAllTiposForStats] = useState<TipoActividadTecnica[]>([]);
    ...
    useEffect(() => {
        const loadAllForStats = async () => {
            ...
            try {
                const data = await apiRequest<any>('/tipos-actividad-tecnica/all?limit=1000&page=1&includeInactive=true', {}, session);
    ...
    return (
        <div className="space-y-6">
            <PageBreadcrumb pageTitle="Gestión de Tipos de Actividad Técnica" />

            {/* Quick Stats Section */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                <StatCard 
                    icon={<WrenchScrewdriverIcon className="w-6 h-6 text-brand-500" />} 
                    label="Total Tipos" 
                    value={stats.total} 
                    delay={0.1}
                />
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-gray-900 p-4 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm">
                <div className="flex items-center gap-3">
                    ...
                    <h2 className="text-lg font-bold text-gray-900 dark:text-white">Tipos de Actividad</h2>
                </div>
