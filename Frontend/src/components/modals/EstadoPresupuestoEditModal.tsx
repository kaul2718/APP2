...
interface Props {
    ...
    onSave: (updatedEstado: EstadoPresupuesto) => void;
}

export default function EstadoPresupuestoEditModal({ isOpen, onClose, estado, onSave }: Props) {
    const { data: session } = useSession();
    const token = session?.accessToken || null;
    const [editando, setEditando] = React.useState<EstadoPresupuesto | null>(estado);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!editando.nombre || typeof editando.nombre !== 'string') {
            toast.error('El nombre del estado debe ser un texto');
            return;
        }
        ...
        if (!editando.descripcion || typeof editando.descripcion !== 'string') {
            toast.error('La descripción debe ser un texto');
            return;
        }
        ...
        try {
            const cambios = {
                ...
                estado: estadoModificado ?? editando.estado
            };

            const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/estados-presupuesto/${editando.id}`, {
                method: "PATCH",
                headers: {
            ...
            onSave(data);
            toast.success("Estado de presupuesto actualizado correctamente");
            onClose();
        } catch (error) {
                        ...
                        <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                            ...
                            <div className="lg:col-span-2">
                                ...
                                <textarea
                                    ...
                                    rows={4}
                                />
                                {editando.descripcion !== estado?.descripcion && (
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    {estadoModificado !== null && estadoModificado !== estado?.estado && (
                                        <div className="mt-2 flex items-start">
                                            <svg
                                                className="h-4 w-4 text-yellow-500 mt-0.5 mr-1 flex-shrink-0"
                                                fill="currentColor"
