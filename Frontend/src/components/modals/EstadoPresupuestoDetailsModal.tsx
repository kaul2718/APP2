...
const InputDisplay = ({
  ...
  return (
    <div>
      <label htmlFor={id} className="block mb-1 font-semibold text-gray-700 dark:text-gray-300 flex items-center">
        {icon}
        {label}
      </label>
      <input
...
export default function EstadoPresupuestoDetailsModal({ isOpen, onClose, estado }: Props) {
  if (!estado) return null;

  const estaActivo = estado.estado;
  const fechaCreacion = new Date(estado.createdAt).toLocaleDateString();
  const fechaActualizacion = new Date(estado.updatedAt).toLocaleDateString();
  const fechaEliminacion = estado.deletedAt 
    ? new Date(estado.deletedAt).toLocaleDateString() 
    : "No eliminado";
  ...
  return (
    ...
    >
      <div className="px-6 py-4 space-y-8 text-sm">
        ...
        {estado.presupuestos && estado.presupuestos.length > 0 && (
          <section>
            <h3 className="mb-4 text-lg font-semibold text-gray-900 dark:text-white">
              Presupuestos asociados
            </h3>
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {estado.presupuestos.map((presupuesto) => (
