...
import { TipoNotificacion } from "@/hooks/useTipoNotificacion";

interface Props {
    isOpen: boolean;
    onClose: () => void;
    tipo: TipoNotificacion | null;
    onSave: (updatedTipo: TipoNotificacion) => void;
}

export default function TipoNotificacionEditModal({ isOpen, onClose, tipo, onSave }: Props) {
    const { data: session } = useSession();
    const token = session?.accessToken || null;
    const [editando, setEditando] = React.useState<TipoNotificacion | null>(tipo);
    const [cargando, setCargando] = React.useState(false);
    const [estadoModificado, setEstadoModificado] = React.useState<boolean | null>(null);
    const [descripcionModificada, setDescripcionModificada] = React.useState<string | null>(null);

    React.useEffect(() => {
        setEditando(tipo);
        setEstadoModificado(null);
        setDescripcionModificada(null);
    }, [tipo]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setCargando(true);
        try {
            const cambios: Partial<TipoNotificacion> = {};

            if (editando.nombre !== tipo?.nombre) cambios.nombre = editando.nombre;
            if (editando.descripcion !== tipo?.descripcion) 
            ...
            if (Object.keys(cambios).length > 0) {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tipos-notificacion/${editando.id}`, {
                    method: "PATCH",
                    headers: {
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleCancel} className="max-w-[700px] m-4" title="Editar Tipo de Notificación">
            <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-10">
                <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
                    Editar información del tipo de notificación
                </h4>
                <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
                    Puedes modificar los datos del tipo de notificación. Los cambios se guardarán al presionar "Guardar cambios".
                </p>
                        ...
                        <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                            ...
                            <div className="lg:col-span-2">
                                ...
                                <textarea
                                    ...
                                    onChange={handleInputChange}
                                    disabled={cargando}
                                    className="w-full px-4 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500 bg-white dark:bg-gray-800 text-black dark:text-white dark:border-gray-600 disabled:opacity-50"
                                    rows={3}
                                />
                                {editando.descripcion !== tipo?.descripcion && (
                                    <div className="mt-2 flex items-start">
                                        ...
                                        <span className="text-sm text-yellow-600">
                                            La descripción será modificada
                                        </span>
                                    </div>
                                )}
                            </div>
