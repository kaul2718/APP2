...
import { TipoEspecificacion } from "@/hooks/useTipoEspecificacion";

interface Props {
    isOpen: boolean;
    onClose: () => void;
    tipo: TipoEspecificacion | null;
    onSave: (updatedTipo: TipoEspecificacion) => void;
}

export default function TipoEspecificacionEditModal({ isOpen, onClose, tipo, onSave }: Props) {
    const { data: session } = useSession();
    const token = session?.accessToken || null;
    const [editando, setEditando] = React.useState<TipoEspecificacion | null>(tipo);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setCargando(true);
        try {
            const cambios: Partial<TipoEspecificacion> = {};

            if (editando.nombre !== tipo?.nombre) cambios.nombre = editando.nombre;
            if (editando.unidad !== tipo?.unidad) cambios.unidad = editando.unidad;
            if (estadoModificado !== null && estadoModificado !== tipo?.estado)
                cambios.estado = estadoModificado;

            if (Object.keys(cambios).length > 0) {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tipo-especificacion/${editando.id}`, {
                    method: "PATCH",
                    headers: {
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleCancel} className="max-w-[700px] m-4" title="Editar Tipo de Especificación">
            <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-10">
                <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
                    Editar información del tipo de especificación
                </h4>
                <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
                    Puedes modificar los datos del tipo de especificación. Los cambios se guardarán al presionar "Guardar cambios".
                </p>
