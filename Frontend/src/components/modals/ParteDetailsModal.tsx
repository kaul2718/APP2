...
export default function ParteDetailsModal({ isOpen, onClose, parte }: Props) {
  if (!parte) return null;

  const estaActivo = parte.estado;
  const fechaCreacion = new Date(parte.createdAt).toLocaleDateString();
  const fechaActualizacion = new Date(parte.updatedAt).toLocaleDateString();
  const fechaEliminacion = parte.deletedAt 
    ? new Date(parte.deletedAt).toLocaleDateString() 
    : "No eliminado";
      ...
      <div className="px-6 py-4 space-y-8 text-sm">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
            ...
            <InputDisplay
              label="Categoría"
              value={parte.categoria?.nombre || "Sin categoría"}
              icon={
                <Icon>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
            ...
            <InputDisplay
              label="Marca"
              value={parte.marca?.nombre || "Sin marca"}
              icon={
                <Icon>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
