...
interface Props {
    ...
    onSave: (updatedEstadoOrden: EstadoOrden) => void;
}

export default function EstadoOrdenEditModal({ isOpen, onClose, estadoOrden, onSave }: Props) {
    const { data: session } = useSession();
    const token = session?.accessToken || null;
    const [editando, setEditando] = React.useState<EstadoOrden | null>(estadoOrden);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setCargando(true);
        try {
            const cambios: Partial<EstadoOrden> = {};

            if (editando.nombre !== estadoOrden?.nombre) cambios.nombre = editando.nombre;
            if (editando.descripcion !== estadoOrden?.descripcion) cambios.descripcion = editando.descripcion;
            if (estadoModificado !== null && estadoModificado !== estadoOrden?.estado)
                cambios.estado = estadoModificado;

            if (Object.keys(cambios).length > 0) {
                const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/estados-orden/${editando.id}`, {
                    method: "PATCH",
                    headers: {
    ...
    return (
        <Modal isOpen={isOpen} onClose={handleCancel} className="max-w-[700px] m-4" title="Editar Estado de Orden">
            <div className="no-scrollbar relative w-full max-w-[700px] overflow-y-auto rounded-3xl bg-white p-4 dark:bg-gray-900 lg:p-10">
                <h4 className="mb-2 text-2xl font-semibold text-gray-800 dark:text-white/90">
                    Editar información del estado de orden
                </h4>
                <p className="mb-6 text-sm text-gray-500 dark:text-gray-400">
                    Puedes modificar los datos del estado de orden. Los cambios se guardarán al presionar "Guardar cambios".
                </p>
                        ...
                        <div className="grid grid-cols-1 gap-x-6 gap-y-5 lg:grid-cols-2">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    {estadoModificado !== null && estadoModificado !== estadoOrden?.estado && (
