...
export default function CategoriaDetailsModal({ isOpen, onClose, categoria }: Props) {
  if (!categoria) return null;

  const estaActivo = categoria.estado;
  const fechaCreacion = new Date(categoria.createdAt).toLocaleDateString();
  const fechaActualizacion = new Date(categoria.updatedAt).toLocaleDateString();
  const fechaEliminacion = categoria.deletedAt 
    ? new Date(categoria.deletedAt).toLocaleDateString() 
    : "No eliminada";
