...
export default function ModeloDetailsModal({ isOpen, onClose, modelo }: Props) {
  if (!modelo) return null;

  const estaActivo = modelo.estado;
  const fechaCreacion = new Date(modelo.createdAt).toLocaleDateString();
  const fechaActualizacion = new Date(modelo.updatedAt).toLocaleDateString();
  const fechaEliminacion = modelo.deletedAt 
    ? new Date(modelo.deletedAt).toLocaleDateString() 
    : "No eliminado";
      ...
      <div className="px-6 py-4 space-y-8 text-sm">
        ...
        <section>
          ...
          <div className="grid grid-cols-1 gap-x-6 gap-y-4 sm:grid-cols-2">
            ...
            <InputDisplay
              label="Marca"
              value={modelo.marca?.nombre || "Sin marca"}
              icon={
                <Icon>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
