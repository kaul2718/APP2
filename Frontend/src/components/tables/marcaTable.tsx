...
export default function MarcaTable() {
  ...
  const handleToggleEstado = async (marca: Marca) => {
    const estaActivo = marca.estado;
    const accion = estaActivo ? "deshabilit" : "habilit";

    const confirmacion = confirm(`¿Estás seguro de ${accion}ar esta marca?`);
      ...
      <div className="p-4 border-b border-gray-100 dark:border-white/[0.05]">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="relative w-full sm:w-96">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSearchTerm(valor);
                    fetchMarcas(1, 10, valor, showDisabled);
                  }}
          ...
          <div className="flex items-center justify-start sm:justify-end w-full text-sm text-gray-500 dark:text-gray-400">
            Mostrando {marcas.length} de {totalItems} marcas
          </div>
        </div>
      </div>
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={4} className="text-center py-4 text-gray-500">
                    {searchTerm ? "No se encontraron resultados" : "No hay marcas disponibles."}
                  </TableCell>
                </TableRow>
              ) : (
                marcas.map((marca) => {
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            aria-label={estaActivo ? "Deshabilitar marca" : "Habilitar marca"}
                          >
                            {estaActivo ? (
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-100 dark:border-white/[0.05]">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => fetchMarcas(currentPage - 1, 10, searchTerm, showDisabled)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => fetchMarcas(page, 10, searchTerm, showDisabled)}
