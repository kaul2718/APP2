...
export default function CategoriaTable() {
  ...
  const handleToggleEstado = async (categoria: Categoria) => {
    ...
    const confirmacion = confirm(`¿Estás seguro de ${accion} esta categoría?`);
    if (!confirmacion) return;

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/categorias/${categoria.id}/toggle-estado`, {
        method: "PATCH",
        headers: {
              ...
              <div className="relative w-full sm:w-96">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSearchTerm(valor);
                    fetchCategorias(1, 10, valor, showInactive);
                  }}
                ...
                {searchTerm && (
                  <button
                    ...
                    onClick={() => {
                      setSearchTerm("");
                      fetchCategorias(1, 10, "", showInactive);
                    }}
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            aria-label={estaActivo ? "Deshabilitar categoría" : "Habilitar categoría"}
                          >
                            {estaActivo ? (
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-100 dark:border-white/[0.05]">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => fetchCategorias(currentPage - 1, 10, searchTerm, showInactive)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => fetchCategorias(page, 10, searchTerm, showInactive)}
