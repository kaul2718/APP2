...
export default function TipoActividadTecnicaTable() {
  ...
  const handleToggleEstado = async (tipo: TipoActividadTecnica) => {
    const estaActivo = tipo.estado;
    const accion = estaActivo ? "deshabilitar" : "habilitar";

    const confirmacion = confirm(`¿Estás seguro de ${accion} este tipo de actividad técnica?`);
    if (!confirmacion) return;

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tipos-actividad-tecnica/${tipo.id}/toggle-estado`, {
        method: "PATCH",
        headers: {
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:gap-x-3 gap-y-2">
              ...
              <div className="relative w-full sm:w-96">
                <input
                  ...
                  aria-label="Buscar tipos de actividad técnica"
                />
                {searchTerm && (
                  <button
      ...
      <div className="max-w-full overflow-x-auto">
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4 text-gray-500">
                    Cargando tipos de actividad técnica...
                  </TableCell>
                </TableRow>
              ) : tipos.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4 text-gray-500">
                    {searchTerm ? "No se encontraron resultados" : "No hay tipos de actividad técnica disponibles."}
                  </TableCell>
                </TableRow>
              ) : (
                tipos.map((tipo) => {
        ...
        {/* Modals */}
        <TipoActividadTecnicaDetailsModal
          isOpen={isModalOpen}
