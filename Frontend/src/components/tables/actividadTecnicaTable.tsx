...
export default function ActividadTecnicaTable() {
    ...
    const handleToggleEstado = async (actividad: ActividadTecnica) => {
        const estaActivo = actividad.estado;
        const accion = estaActivo ? "deshabilitar" : "habilitar";

        const confirmacion = confirm(`¿Estás seguro de ${accion} esta actividad técnica?`);
        if (!confirmacion) return;

        try {
            const response = await fetch(
                `${process.env.NEXT_PUBLIC_BACKEND_URL}/actividades-tecnicas/${actividad.id}/toggle-estado`,
                {
                    method: "PATCH",
                    headers: {
            ...
            console.error(`Error al ${accion} actividad técnica:`, error);
            toast.error(`Error al ${accion} actividad técnica`);
        }
    };
                        ...
                        <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:gap-x-3 gap-y-2">
                            ...
                            <div className="relative w-full sm:w-96">
                                <input
                                    id="buscarActividad"
                                    type="text"
                                    placeholder="Por diagnóstico o trabajo realizado..."
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setSearchTerm(valor);
                                        fetchActividades(1, 10, valor, showInactive);
                                    }}
                                    aria-label="Buscar actividades técnicas"
                                />
                                {searchTerm && (
                                    <button
                                        ...
                                        onClick={() => {
                                            setSearchTerm("");
                                            fetchActividades(1, 10, "", showInactive);
                                        }}
                    ...
                    <Table>
                        ...
                        <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
                            {loading ? (
                                ...
                                <TableRow>
                                    <TableCell colSpan={6} className="text-center py-4 text-gray-500">
                                        {searchTerm ? "No se encontraron resultados" : "No hay actividades disponibles."}
                                    </TableCell>
                                </TableRow>
                            ) : (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <div>
                                                        <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                                            ORD-{actividad.orden.workOrderNumber}
                                                        </span>
                                                    </div>
                                                </div>
                                            </TableCell>
                                            ...
                                            <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <button
                                                        ...
                                                        aria-label={estaActivo ? "Deshabilitar actividad" : "Habilitar actividad"}
                                                    >
                                                        {estaActivo ? (
                                                            <svg
                                                                xmlns="http://www.w3.org/2000/svg"
                ...
                {totalPages > 1 && (
                    <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-100 dark:border-white/[0.05]">
                        ...
                        <div className="flex items-center gap-2">
                            <button
                                onClick={() => fetchActividades(currentPage - 1, 10, searchTerm, showInactive)}
                            ...
                            {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                                <button
                                    key={page}
                                    onClick={() => fetchActividades(page, 10, searchTerm, showInactive)}
