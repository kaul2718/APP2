...
export default function EstadoPresupuestoTable() {
  ...
  const handleToggleEstado = async (estado: EstadoPresupuesto) => {
    const estaActivo = estado.estado;
    const accion = estaActivo ? "deshabilitar" : "habilitar";

    const confirmacion = confirm(`¿Estás seguro de ${accion} este estado de presupuesto?`);
    if (!confirmacion) return;

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/estados-presupuesto/${estado.id}/toggle-estado`, {
        method: "PATCH",
        headers: {
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="relative w-full sm:w-96">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSearchTerm(valor);
                    fetchEstados(1, 10, valor, showInactive);
                  }}
                  aria-label="Buscar estados de presupuesto"
                />
                {searchTerm && (
                  <button
                    ...
                    onClick={() => {
                      setSearchTerm("");
                      fetchEstados(1, 10, "", showInactive);
                    }}
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              Mostrando {estados.length} de {totalItems} estados
            </div>
          </div>
        </div>
      </div>
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4 text-gray-500">
                    Cargando estados de presupuesto...
                  </TableCell>
                </TableRow>
              ) : estados.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4 text-gray-500">
                    {searchTerm ? "No se encontraron resultados" : "No hay estados de presupuesto disponibles."}
                  </TableCell>
                </TableRow>
              ) : (
                estados.map((estado) => {
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            title="Editar"
                            aria-label="Editar estado de presupuesto"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              className="w-5 h-5"
                          ...
                          <button
                            ...
                            aria-label={estaActivo ? "Deshabilitar estado de presupuesto" : "Habilitar estado de presupuesto"}
                          >
                            {estaActivo ? (
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-100 dark:border-white/[0.05]">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => fetchEstados(currentPage - 1, 10, searchTerm, showInactive)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => fetchEstados(page, 10, searchTerm, showInactive)}
