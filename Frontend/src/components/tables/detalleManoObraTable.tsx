...
import { useDetalleManoObra } from "@/hooks/useDetalleManoObra";
import { CurrencyDollarIcon, DocumentTextIcon, HashtagIcon, UserIcon } from "@heroicons/react/24/outline";
...
export default function DetalleManoObraTable({ presupuestoId }: { presupuestoId?: number }) {
    ...
    const handleToggleEstado = async (detalle: any) => {
        ...
        const confirmacion = confirm(`¿Estás seguro de ${accion} este detalle?`);
                            ...
                            {loading ? (
                                <TableRow>
                                    <TableCell colSpan={presupuestoId ? 7 : 8} className="text-center py-4 text-gray-500">
                                        Cargando detalles de mano de obra...
                                    </TableCell>
                                </TableRow>
                            ) : detalles.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={presupuestoId ? 7 : 8} className="text-center py-4 text-gray-500">
                                        {searchTerm ? "No se encontraron resultados" : "No hay detalles de mano de obra disponibles."}
                                    </TableCell>
                                </TableRow>
                            ) : (
                                                ...
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <span className="block font-medium text-gray-800 text-theme-sm dark:text-white/90">
                                                            {detalle.tipoManoObra?.nombre || "Tipo no disponible"}
                                                        </span>
                                                        <span className="block text-xs text-gray-500 dark:text-gray-400">
                                                            {detalle.tipoManoObra?.codigo || "Sin código"}
                                                        </span>
                                                    </div>
                                                </div>
                                            </TableCell>
                    ...
                    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                        <div className="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-md w-full">
                            ...
                            <p className="text-gray-600 dark:text-gray-300 mb-6">
                                ¿Estás seguro de que deseas eliminar este detalle de mano de obra?
                            </p>
