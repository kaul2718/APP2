...
import RepuestoDetailsModal from "../modals/RepuestoDetailsModal";
import RepuestoEditModal from "../modals/RepuestoEditModal";
import { useSession } from "next-auth/react";
import { toast } from "react-toastify";
import { Repuesto, useRepuesto } from "@/hooks/useRepuesto";
import { CurrencyDollarIcon } from "@heroicons/react/24/outline";

export default function RepuestoTable() {
  const {
    repuestos,
    loading,
    setRepuestos,
  ...
  const handleViewClick = (repuesto: Repuesto) => {
    setSelectedRepuesto(repuesto);
    setIsModalOpen(true);
  };

  const handleEditClick = (repuesto: Repuesto) => {
    setSelectedRepuesto(repuesto);
  ...
  const handleSaveRepuesto = (repuestoActualizado: Repuesto) => {
    setRepuestos((prev) => prev.map((r) => (r.id === repuestoActualizado.id ? repuestoActualizado : r)));
    fetchRepuestos(currentPage, 10, searchTerm, showInactive);
  };

  const handleToggleEstado = async (repuesto: Repuesto) => {
    const estaActivo = repuesto.estado;
    const accion = estaActivo ? "deshabilitar" : "habilitar";

    const confirmacion = confirm(`¿Estás seguro de ${accion} este repuesto?`);
    if (!confirmacion) return;

    try {
      await toggleRepuestoStatus(repuesto.id);
      toast.success(`Repuesto ${estaActivo ? 'deshabilitado' : 'habilitado'} correctamente`);
      fetchRepuestos(currentPage, 10, searchTerm, showInactive);
    } catch (error) {
      console.error(`Error al ${accion} repuesto:`, error);
      toast.error(`Error al ${accion} repuesto`);
    }
  };
              ...
              <div className="relative w-full sm:w-96">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSearchTerm(valor);
                    fetchRepuestos(1, 10, valor, showInactive);
                  }}
                ...
                {searchTerm && (
                  <button
                    ...
                    onClick={() => {
                      setSearchTerm("");
                      fetchRepuestos(1, 10, "", showInactive);
                    }}
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-4 text-gray-500">
                    {searchTerm ? "No se encontraron resultados" : "No hay repuestos disponibles."}
                  </TableCell>
                </TableRow>
              ) : (
                    ...
                    <TableRow key={repuesto.id}>
                      ...
                      <TableCell className="px-5 py-4 sm:px-6 text-start">
                        <span className="text-gray-500 dark:text-gray-400">
                          {repuesto.parte?.nombre || 'Sin parte'}
                        </span>
                      </TableCell>
                      <TableCell className="px-5 py-4 sm:px-6 text-start">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-gray-800 dark:text-white/90">
                            {repuesto.precioVenta.toLocaleString('es-CL', {
                              style: 'currency',
                              currency: 'CLP'
                            })}
                          </span>
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center gap-4">
                          {/* Ver */}
                          <button
                            onClick={() => handleViewClick(repuesto)}
                          ...
                          <button
                            ...
                            onClick={() => handleToggleEstado(repuesto)}
                            title={estaActivo ? "Deshabilitar" : "Habilitar"}
                            aria-label={estaActivo ? "Deshabilitar repuesto" : "Habilitar repuesto"}
                          >
                            {estaActivo ? (
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-100 dark:border-white/[0.05]">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => fetchRepuestos(currentPage - 1, 10, searchTerm, showInactive)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => fetchRepuestos(page, 10, searchTerm, showInactive)}
