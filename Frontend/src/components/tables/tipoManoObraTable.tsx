...
export default function TipoManoObraTable() {
  ...
  const handleToggleEstado = async (tipo: TipoManoObra) => {
    const estaActivo = tipo.estado;
    const accion = estaActivo ? "deshabilitar" : "habilitar";

    const confirmacion = confirm(`¿Estás seguro de ${accion} este tipo de mano de obra?`);
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:gap-x-3 gap-y-2">
              ...
              <div className="relative w-full sm:w-96">
                <input
                  ...
                  aria-label="Buscar tipos de mano de obra"
                />
                {searchTerm && (
                  <button
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-4 text-gray-500">
                    Cargando tipos de mano de obra...
                  </TableCell>
                </TableRow>
              ) : tipos.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-4 text-gray-500">
                    {searchTerm ? "No se encontraron resultados" : "No hay tipos de mano de obra disponibles."}
                  </TableCell>
                </TableRow>
              ) : (
                tipos.map((tipo) => {
