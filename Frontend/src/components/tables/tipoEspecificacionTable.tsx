...
import TipoEspecificacionDetailsModal from "../modals/TipoEspecificacionDetailsModal";
import TipoEspecificacionEditModal from "../modals/TipoEspecificacionEditModal";
import { useSession } from "next-auth/react";
import { toast } from "react-toastify";
import { TipoEspecificacion, useTipoEspecificacion } from "@/hooks/useTipoEspecificacion";

export default function TipoEspecificacionTable() {
  const {
    tipos,
    loading,
    setTipos,
    ...
    showInactive,
    setShowInactive,
  } = useTipoEspecificacion();

  const { data: session } = useSession();
  const token = session?.accessToken || "";

  const [selectedTipo, setSelectedTipo] = useState<TipoEspecificacion | null>(null);
  const [isModalOpen, setIsModalOpen] = useState(false);
  const [isEditModalOpen, setIsEditModalOpen] = useState(false);

  const handleViewClick = (tipo: TipoEspecificacion) => {
    setSelectedTipo(tipo);
    setIsModalOpen(true);
  };

  const handleEditClick = (tipo: TipoEspecificacion) => {
    setSelectedTipo(tipo);
  ...
  const handleSaveTipo = (tipoActualizado: TipoEspecificacion) => {
    setTipos((prev) => prev.map((t) => (t.id === tipoActualizado.id ? tipoActualizado : t)));
    fetchTipos(currentPage, 10, searchTerm, showInactive);
  };

  const handleToggleEstado = async (tipo: TipoEspecificacion) => {
    const estaActivo = tipo.estado;
    const accion = estaActivo ? "deshabilitar" : "habilitar";

    const confirmacion = confirm(`¿Estás seguro de ${accion} este tipo de especificación?`);
    if (!confirmacion) return;

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tipo-especificacion/${tipo.id}/toggle-estado`, {
        method: "PATCH",
        headers: {
      ...
      toast.success(`Tipo de especificación ${estaActivo ? 'deshabilitado' : 'habilitado'} correctamente`);
      fetchTipos(currentPage, 10, searchTerm, showInactive);
    } catch (error) {
      console.error(`Error al ${accion} tipo de especificación:`, error);
      toast.error(`Error al ${accion} tipo de especificación`);
    }
  };
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-gray-200 bg-white dark:border-white/[0.05] dark:bg-white/[0.03]">
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
              ...
              <div className="relative w-full sm:w-96">
                <input
                  ...
                  onChange={(e) => {
                    ...
                    setSearchTerm(valor);
                    fetchTipos(1, 10, valor, showInactive);
                  }}
                  aria-label="Buscar tipos de especificación"
                />
                {searchTerm && (
                  <button
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="text-sm text-gray-500 dark:text-gray-400">
              Mostrando {tipos.length} de {totalItems} tipos
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-full overflow-x-auto">
              ...
              {loading ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4 text-gray-500">
                    Cargando tipos de especificación...
                  </TableCell>
                </TableRow>
              ) : tipos.length === 0 ? (
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4 text-gray-500">
                    {searchTerm ? "No se encontraron resultados" : "No hay tipos de especificación disponibles."}
                  </TableCell>
                </TableRow>
              ) : (
                tipos.map((tipo) => {
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            aria-label={estaActivo ? "Deshabilitar tipo" : "Habilitar tipo"}
                          >
                            {estaActivo ? (
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-100 dark:border-white/[0.05]">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => fetchTipos(currentPage - 1, 10, searchTerm, showInactive)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => fetchTipos(page, 10, searchTerm, showInactive)}
        ...
        {/* Modals */}
        <TipoEspecificacionDetailsModal
          isOpen={isModalOpen}
          onClose={handleCloseModal}
          tipo={selectedTipo}
        />
        <TipoEspecificacionEditModal
