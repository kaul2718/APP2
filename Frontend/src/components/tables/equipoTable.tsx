...
export default function EquipoTable() {
  ...
  const handleToggleEstado = async (equipo: Equipo) => {
    ...
    const confirmacion = confirm(`¿Estás seguro de ${accion} este equipo?`);
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/equipos/${equipo.id}/toggle-estado`,
        {
          ...
          body: JSON.stringify({ estado: !estaActivo }), // Envía el nuevo estado
        }
      );
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:gap-x-3 gap-y-2">
              ...
              <div className="relative w-full sm:w-96">
                <input
                  id="buscarEquipo"
                  type="text"
                  placeholder="Por número de serie..."
                  ...
                  onChange={(e) => {
                    ...
                    setSearchTerm(valor);
                    fetchEquipos(1, 10, valor, showInactive);
                  }}
                ...
                {searchTerm && (
                  <button
                    ...
                    onClick={() => {
                      setSearchTerm("");
                      fetchEquipos(1, 10, "", showInactive);
                    }}
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-4 text-gray-500">
                    {searchTerm ? "No se encontraron resultados" : "No hay equipos disponibles."}
                  </TableCell>
                </TableRow>
              ) : (
                equipos.map((equipo) => {
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            aria-label={estaActivo ? "Deshabilitar equipo" : "Habilitar equipo"}
                          >
                            {estaActivo ? (
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
        ...
        {totalPages > 1 && (
          <div className="flex flex-col sm:flex-row items-center justify-between p-4 border-t border-gray-100 dark:border-white/[0.05]">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={() => fetchEquipos(currentPage - 1, 10, searchTerm, showInactive)}
              ...
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button
                  key={page}
                  onClick={() => fetchEquipos(page, 10, searchTerm, showInactive)}
