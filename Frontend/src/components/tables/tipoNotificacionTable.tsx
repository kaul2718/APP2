...
export default function TipoNotificacionTable() {
  ...
  const handleToggleEstado = async (tipo: TipoNotificacion) => {
    const estaActivo = tipo.estado;
    const accion = estaActivo ? "deshabilitar" : "habilitar";

    const confirmacion = confirm(`¿Estás seguro de ${accion} este tipo de notificación?`);
    if (!confirmacion) return;

    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tipos-notificacion/${tipo.id}/toggle-estado`, {
        method: "PATCH",
        headers: {
      ...
      toast.success(`Tipo de notificación ${estaActivo ? 'deshabilitado' : 'habilitado'} correctamente`);
      fetchTipos(currentPage, 10, searchTerm, showInactive);
    } catch (error) {
      console.error(`Error al ${accion} tipo de notificación:`, error);
      toast.error(`Error al ${accion} tipo de notificación`);
    }
  };
            ...
            <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:gap-x-3 gap-y-2">
              ...
              <div className="relative w-full sm:w-96">
                <input
                  ...
                  aria-label="Buscar tipos de notificación"
                />
                {searchTerm && (
                  <button
          ...
          <Table>
            ...
            <TableBody className="divide-y divide-gray-100 dark:divide-white/[0.05]">
              {loading ? (
                ...
                <TableRow>
                  <TableCell colSpan={5} className="text-center py-4 text-gray-500">
                    {searchTerm ? "No se encontraron resultados" : "No hay tipos de notificación disponibles."}
                  </TableCell>
                </TableRow>
              ) : (
                tipos.map((tipo) => {
                      ...
                      <TableCell className="px-4 py-3 text-gray-500 text-start text-theme-sm dark:text-gray-400">
                        <div className="flex items-center gap-4">
                          ...
                          <button
                            ...
                            title="Editar"
                            aria-label="Editar tipo de notificación"
                          >
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              className="w-5 h-5"
                          ...
                          <button
                            ...
                            aria-label={estaActivo ? "Deshabilitar tipo de notificación" : "Habilitar tipo de notificación"}
                          >
                            {estaActivo ? (
                              <svg
                                xmlns="http://www.w3.org/2000/svg"
